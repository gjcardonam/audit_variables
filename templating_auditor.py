...
1) Las dos primeras variables deben ser, en orden: 'schema', 'groupId'.
   - Ambas ocultas (hide != 0).

2) Desde la 3.ª variable hasta antes de encontrar 'some_nodeId' / 'all_nodeId' / 'nodeId'
   son "filters" (Area, County, Battery, Formation, ...). Para cada filter:
   - visible (hide == 0)
   - multi: true
   - includeAll: true
   - sort == 1
   - current en "All" / "$__all"

3) A partir de 'some_nodeId' / 'all_nodeId' / 'nodeId' (incluyendo el resto):
   - deben estar ocultas (hide != 0), salvo reglas específicas:
     * some_nodeId: visible, multi/includeAll true, current All.
     * all_nodeId: oculta, includeAll true, current All.
     * nodeId: oculta, includeAll false.

4) Variable especial: 'scale' debe estar visible.

SALIDAS
-------
- Por ambiente: templating_audit_{env}.json / templating_audit_{env}.csv
- Global: templating_audit_all.json
- Opcional: inserción en PostgreSQL (templating_audit_runs, templating_audit_violations)

Control de salidas (archivos)
-----------------------------
- Config JSON:
  "output": { "write_json": true, "write_csv": true }

- Variables de entorno (OVERRIDES):
  WRITE_JSON = 0|1|true|false|no|yes
  WRITE_CSV  = 0|1|true|false|no|yes

Si WRITE_JSON/WRITE_CSV están definidos en el entorno, mandan sobre el config.
"""
...
def _resolve_grafana_fields(env_block: dict) -> Dict[str, Any]:
    ...
    verify_ssl   = _bool_env_default(
        os.getenv("VERIFY_SSL", g.get("verify_ssl", True)),
        True
    )
...
def build_config(args) -> Dict[str, Any]:
    # Cargar variables desde .env si existe (para uso local)
    try:
...
def validate_variables(vars_list: List[Dict[str, Any]]) -> List[Dict[str, Optional[str]]]:
    ...
    # primeras dos variables: schema y groupId
    if len(vars_list) < 2:
    ...
    if v0 != "schema":
        add("first_var_must_be_schema", "schema")
    if v1 != "groupId":
        add("second_var_must_be_groupId", "groupId")
    ...
    for i, v in enumerate(vars_list):
        ...
        # saltar schema / groupId
        if n_norm in ("schema", "groupid"):
            continue

        # variable especial: scale -> visible
        if n_norm == "scale":
            if is_hidden(v):
        ...
        if n_norm == "some_nodeid":
            if is_hidden(v):
                add("some_nodeId_must_be_visible", n_raw or "some_nodeId")
            if v.get("multi") is not True:
                add("some_nodeId_multi_true", n_raw or "some_nodeId")
            if v.get("includeAll") is not True:
                add("some_nodeId_includeAll_true", n_raw or "some_nodeId")
        ...
        if n_norm == "all_nodeid":
            if not is_hidden(v):
                add("nonfilter_all_nodeId_must_be_hidden", n_raw or "all_nodeId")
            if v.get("includeAll") is not True:
                add("all_nodeId_includeAll_true", n_raw or "all_nodeId")
            cur = v.get("current", {}) or {}
            txt = as_list(cur.get("text"))
            val = as_list(cur.get("value"))
            if not (("All" in txt) and ("$__all" in val)):
        ...
        if n_norm == "nodeid":
            if not is_hidden(v):
                add("nonfilter_nodeId_must_be_hidden", n_raw or "nodeId")
            if v.get("includeAll") is True:
                add("nodeId_includeAll_must_be_false", n_raw or "nodeId")
            continue

        # filtros: después de groupId y antes del primer *_nodeId
        is_filter = (idx_group is not None) and (i > idx_group) and (i < cut_idx)

        if is_filter:
            if is_hidden(v):
                add(f"filter_{n_raw}_must_be_visible", n_raw)
            if v.get("multi") is not True:
                add(f"filter_{n_raw}_multi_true", n_raw)
            if v.get("includeAll") is not True:
                add(f"filter_{n_raw}_includeAll_true", n_raw)
            if as_int(v.get("sort"), 0) != 1:
                add(f"filter_{n_raw}_sort_must_be_1", n_raw)
            cur = v.get("current", {}) or {}
            txt = as_list(cur.get("text"))
            val = as_list(cur.get("value"))
            if not (("All" in txt) and ("$__all" in val)):
...
""")
    ...
    # BD: guardar violaciones y cerrar
    if db_cfg["enabled"]:
        with conn.cursor() as cur:
            if violations:
            ...
            cur.execute(
                f"""UPDATE {db_cfg['schema']}.templating_audit_runs
                    SET ended_at=now(), violations_count=%s, elapsed_seconds=%s
                    WHERE run_id=%s""",
...
def main():
    ap = argparse.ArgumentParser(
        description="Audita VARIABLES (templating) de Grafana por ambientes/compañías."
    )
